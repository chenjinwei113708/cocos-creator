...
cc.Class({
    ...
    onLoad () {
        ...
        this.gameInfo = {
            ...
            ppcardSize: {width: 66, height: 90.1},
            fruitSize: {width: 90, height: 90},
        };
        // 不同类型对应的图片
        this.sprites = {
        ...
        // 各个坐标对应的方块，下标0不用，左上角坐标为(1, 1), 顶部为第一行，第一行第二个的坐标为 (1, 2)
        this.uicells = [
            [undefined, ...this.uibox.getChildByName('a').children], // 这一行是看不见的
            [undefined, ...this.uibox.getChildByName('b').children],
            [undefined, ...this.uibox.getChildByName('c').children],
            [undefined, ...this.uibox.getChildByName('d').children],
            [undefined, ...this.uibox.getChildByName('e').children],
            [undefined, ...this.uibox.getChildByName('f').children],
        ];
        ...
        this.actionDown = {type: ACTION_TYPE.DOWN, nodes: [
            {start: cc.v2(0,1), end: cc.v2(5,1), newType: CELL_TYPE.GRAPE},
            {start: cc.v2(0,1), end: cc.v2(5,2), newType: CELL_TYPE.BANANA},
            {start: cc.v2(0,1), end: cc.v2(5,3), newType: CELL_TYPE.CARROT},
            {start: cc.v2(0,1), end: cc.v2(5,4), newType: CELL_TYPE.CARROT},
            {start: cc.v2(0,1), end: cc.v2(5,5), newType: CELL_TYPE.GRAPE},
            {start: cc.v2(0,1), end: cc.v2(4,1), newType: CELL_TYPE.GRAPE},
            {start: cc.v2(0,1), end: cc.v2(4,2), newType: CELL_TYPE.PEAR},
            {start: cc.v2(0,1), end: cc.v2(4,3), newType: CELL_TYPE.STRAWBERRY},
            {start: cc.v2(0,1), end: cc.v2(4,4), newType: CELL_TYPE.GRAPE},
            {start: cc.v2(0,1), end: cc.v2(4,5), newType: CELL_TYPE.STRAWBERRY},
            {start: cc.v2(0,1), end: cc.v2(3,1), newType: CELL_TYPE.PEAR},
            {start: cc.v2(0,1), end: cc.v2(3,2), newType: CELL_TYPE.STRAWBERRY},
            {start: cc.v2(0,1), end: cc.v2(3,3), newType: CELL_TYPE.PEAR},
            {start: cc.v2(0,1), end: cc.v2(3,4), newType: CELL_TYPE.PEAR},
            {start: cc.v2(0,1), end: cc.v2(3,5), newType: CELL_TYPE.GRAPE},
            {start: cc.v2(0,1), end: cc.v2(2,1), newType: CELL_TYPE.STRAWBERRY},
            {start: cc.v2(0,1), end: cc.v2(2,2), newType: CELL_TYPE.GRAPE},
            {start: cc.v2(0,1), end: cc.v2(2,3), newType: CELL_TYPE.GRAPE},
            {start: cc.v2(0,1), end: cc.v2(2,4), newType: CELL_TYPE.APPLE},
            {start: cc.v2(0,1), end: cc.v2(2,5), newType: CELL_TYPE.BANANA},
            {start: cc.v2(0,1), end: cc.v2(1,1), newType: CELL_TYPE.CARROT},
            {start: cc.v2(0,1), end: cc.v2(1,2), newType: CELL_TYPE.CARROT},
            {start: cc.v2(0,1), end: cc.v2(1,3), newType: CELL_TYPE.PEAR},
            {start: cc.v2(0,1), end: cc.v2(1,4), newType: CELL_TYPE.STRAWBERRY},
            {start: cc.v2(0,1), end: cc.v2(1,5), newType: CELL_TYPE.PEAR},
            ]
        };
    ...
    doActions () {
        ...
        if (this.actionList.length > 0) {
            ...
            setTimeout(() => {this.becomePPCard();}, 100);
            // this.changeToNextLevel();
            // this.setCellStatus(CELL_STATUS.CAN_MOVE);
        }
    },
    ...
    /**全部变成pp卡飞到顶部 */
    becomePPCard () {
        let paypal = this.gameController.guideView.paypal;
        ...
        let callnum = 0;
        let callnum2 = 0;
        this.uibox.active = true;
        for (let i = 1; i <= 5; i++) {
            for (let j = 1; j <= 5; j++) {
                let card = this.cells[i][j];
                let ppcard = this.uicells[i][j];
                ppcard.opacity = 0;
                card.runAction(cc.sequence(
                    cc.repeat(cc.sequence(cc.rotateTo(0.09, 11), cc.rotateTo(0.09, -11)), 3),
                    cc.scaleTo(0.2, 0),
                    cc.callFunc(() => {
                        let moveTime = Math.random()*0.9;
                        let angle = -60+Math.random()*120;
                        card.opacity = 0;
                        card.scale = 1;
                        ppcard.getComponent(cc.Sprite).spriteFrame = this.spritePPcard;
                        ppcard.width = this.gameInfo.ppcardSize.width;
                        ppcard.height = this.gameInfo.ppcardSize.height;
                        ppcard.position = cc.v2(card.position.x, card.position.y);
                        ppcard.angle = 0;
                        callnum++;
                        ppcard.runAction(cc.spawn(cc.scaleTo(0.1, 1), cc.fadeIn(0.1)));
                        setTimeout(() => {
                            ppcard.runAction(cc.sequence(
                                cc.sequence(cc.spawn(cc.scaleTo(0.2, 1.2), cc.rotateTo(0.3, -15)), cc.spawn(cc.scaleTo(0.2, 0.9),cc.rotateTo(0.2, 15))).easing(cc.easeInOut(2.1)),
                                cc.spawn(cc.scaleTo(0.3, 1), cc.rotateTo(0.3, 0)),
                                cc.spawn(cc.moveTo(moveTime, destPos), cc.fadeTo(moveTime, 100), cc.rotateTo(moveTime, angle)).easing(cc.easeOut(1.6)),
                                cc.fadeOut(0.1),
                                cc.callFunc(() => {
                                    callnum2++;
                                    if (callnum2 === 25) {
                                        console.log('结束');
                                        this.uibox.active = false;
                                        this.gameController.addCash(100);
                                        this.fallDownCards();
                                    }
                                })
    ...
    /**降落所有卡片 */
    fallDownCards () {
        let action = this.actionDown;
        action.nodes.forEach((item, index) => {
            let done = false;
            // if (index >= action.nodes.length-1) {done = true;}
            setTimeout(() => {this.actDown(item.start, item.end, item.newType, done);}, 10*index);
        });
        setTimeout(() => {
            this.changeToNextLevel();
            this.setCellStatus(CELL_STATUS.CAN_MOVE);
