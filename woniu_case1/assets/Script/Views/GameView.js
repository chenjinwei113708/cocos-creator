...
cc.Class({
    ...
    change2Battle () {
        ...
        this.info.currentSnail.width = 200;
        this.info.currentSnail.height = this.info.currentSnail.height * scale;
        this.info.currentSnail.width = -this.info.currentSnail.width; // 反转
        this.info.currentSnail.position = cc.v2(-600, -250);
        this.info.currentSnail.parent = model
        console.log(this.info.currentSnail.width, this.info.currentSnail.height)
        ...
        this.battle.runAction(cc.sequence(
            ...
            cc.callFunc(() => {
                this.setGameScene(GAME_SCENE.BATTLE); // 设置游戏场景为battle
                ...
                model.children.forEach((node, index) => {
                    const endX = [180, -180];
                    const buffer = 10; // 缓冲
                    console.log(index ? buffer : -1 * buffer);
                    const endPos = cc.v2(endX[index], -250);
                    console.log(index ? buffer : -1 * buffer + endPos.x, endPos.y, endPos)
                    node.active = true;
