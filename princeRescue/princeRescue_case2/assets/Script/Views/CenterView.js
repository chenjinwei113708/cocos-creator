...
cc.Class({
    ...
    orientCb(isLoad) {
        ...
        // console.log(this.gameModel.isLandscape)
        // 设置开始时的横竖屏
        // if (!firstLandscape) {
        //     firstLandscape = this.gameModel.isLandscape
        // }
        ...
        this.setGameCamera(this.gameModel.isLandscape);
    },

    setGameCamera (isLandscape) {
        if (isLandscape && firstLandscape !== isLandscape) {
            console.log('横屏');
            const gameCamera = cc.find('Canvas/center/GameCamera');
            gameCamera.x = -575;
            gameCamera.y = 165;
        } else if (!isLandscape && firstLandscape !== isLandscape) {
            console.log('竖屏');
            const gameCamera = cc.find('Canvas/center/GameCamera');
            gameCamera.x = 210;
            gameCamera.y = -255
        ...
        Object.keys(elemConf).forEach((key) => {
            switch (key) {
                ...
                case 'scale':
                    ...
                    break;
                case 'zoomRatio': // 摄像头缩放
                    cc.find(path).getComponent(cc.Camera).zoomRatio = elemConf[key];
