...
function flyTo(node1, node2, cb) {
  return new Promise((resolve, reject) => {
    ...
    const minScale = 0.6;
    const endPos = node1.parent.convertToNodeSpaceAR(
...
function blink(node, cb) {
  return new Promise((resolve, reject) => {
    ...
    const blinkTime = 0.1; // 闪烁一次的事件
    const blinkTimes = 3; // 闪烁次数
    const blinkArr = [];
...
function toggleMask(node, type) {
  return new Promise((resolve, reject) => {
    ...
    if (
      node.opacity !== 0 &&
      (type === "out" || (type === undefined && isActive === true))
    ) {
      // 隐藏
      node.stopAllActions();
      node.runAction(
