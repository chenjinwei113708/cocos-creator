...
cc.Class({
    ...
    updateHandByParent(node) {
        ...
        this.hand.position = node.position;
        oriPos && (oriPos = cc.v2(this.hand.position.x, this.hand.position.y)); // 用于后面的方法来表示确认的位置
    },
    ...
    showhand(node, type = 'parent') {
        ...
        // 获取原始坐标
        let oriPos = cc.v2(this.hand.positioncc.x, this.hand.position.y);
        // 运动时移动的动画
