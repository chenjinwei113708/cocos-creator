...
cc.Class({
    ...
    onTouchStart (event) {
        if (this.info.status === GAME_STATUS.CAN_CLICK) {
            ...
            if (this.info.clickTimes === 3) {
                ...
                setTimeout(() => {
                    this.info.getMoney = 2000; // 这次pp卡展示的钱
                    this.showPPcard(this.info.getMoney);
                    this.info.addMoney = '1000'; // 下次点击树增加的钱
                }, 1000);
                ...
                this.treeView.changeToNextTree(); // 变成下一棵树
                setTimeout(() => {
                    this.info.getMoney = 3000;
                    this.showPPcard(this.info.getMoney);
                    this.info.addMoney = '2000';
                }, 1000);
     ...
     */
    showPPcard (num) {
        this.ppcard.getChildByName('money').getComponent(cc.Label).string = `+${num}円`;
